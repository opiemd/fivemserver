            ...
            if locked == 1 or class == 15 or class == 16 or class == 14 then
              ...
              if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
                CloseToVehicle = true
                  ...
                  if vehicleWeight[string.lower(model)] ~= nil then
                      model = string.lower(model)
                      vehCapacity = vehicleWeight[model]
                      --ESX.ShowNotification('weight is '..vehCapacity)
                  else
                      vehCapacity = ConfigVehicleTrunk.VehicleLimit[class]
                  end
                OpenCoffreInventoryMenu(GetVehicleNumberPlateText(vehFront), vehCapacity, myVeh)
              end
            else
