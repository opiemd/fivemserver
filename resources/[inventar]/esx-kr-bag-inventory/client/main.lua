...
function Bag()
    ...
    table.insert(elements, {label = 'Reinpacken', value = 'put'})
    table.insert(elements, {label = 'Rausnehmen', value = 'take'})
    table.insert(elements, {label = 'Tasche Ablegen', value = 'drop'})
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(0, Keys['N']) and HasBag and not IsPedInAnyVehicle(GetPlayerPed(-1), true) and not IsEntityInAir(PlayerPedId()) then -- Change F5 to the key you want to open the meny with
            Bag()
        end
    end
end)
