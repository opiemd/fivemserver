...
ESX.RegisterServerCallback("suku:getShopItems", function(source, cb, shoptype)
	...
	for i=1, #itemResult, 1 do
		...
		if shoptype == "bennys" then
			for _, v in pairs(Config.Shops.Bennys.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
		...
		if shoptype == "mechanic" then
			for _, v in pairs(Config.Shops.Mechanic.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
		...
		if shoptype == "medic" then
			for _, v in pairs(Config.Shops.Medic.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
		...
		if shoptype == "autohaus" then
			for _, v in pairs(Config.Shops.Autohaus.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
		...
		if shoptype == "opie" then
			for _, v in pairs(Config.Shops.Opie.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
		...
		if shoptype == "police" then
			for _, v in pairs(Config.Shops.Police.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
		...
		if shoptype == "bahamamamas" then
			for _, v in pairs(Config.Shops.Bahamamamas.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
		...
		if shoptype == "phoneshop" then
			for _, v in pairs(Config.Shops.PhoneShop.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
            ...
            for i = 1, #list, 1 do
				if list[i].name == item then
					...
					if xPlayer.getMoney() >= totalPrice then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Folgendes hast du gekauft '..count.." "..list[i].label })
					else
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Du hast nicht genug Geld!' })
					end
				end
            end
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Du hast nicht genug Platz in deinem Inventar!' })
				...
				if list[i].name == item then
					...
					if not targetWeapon then
						...
						if xPlayer.getMoney() >= totalPrice then
							...
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Flgendes hast du gekauft '..list[i].label })
						else
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Du hast nicht genug Geld!' })
						end
					else
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Du besitzt diese Waffe bereits!' })
					end
				end
            end
        else
			...
			if list[i].name == item then
				...
				if targetWeapon then
					...
					if xPlayer.getMoney() >= totalPrice then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Flgendes hast du gekauft '..count.." "..list[i].label })

					else
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Du hast nicht genug Geld!' })
					end
				else
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Du besitzt die Waffe für diesen Munitionstyp nicht!' })
				end
            end
        end
    end
...
AddEventHandler('suku:buyLicense', function ()
	...
	if xPlayer.get('money') >= Config.LicensePrice then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Du hast eine Schusswaffenlizenz gekauft.' })
...
ESX.RegisterUsableItem('coffee', function(source)
	...
	TriggerClientEvent('esx_status:add', source, 'thirst', 4000)
	TriggerClientEvent('esx_status:add', source, 'drunk', -1000)
	TriggerClientEvent('esx_basicneeds:onDrink', source)
