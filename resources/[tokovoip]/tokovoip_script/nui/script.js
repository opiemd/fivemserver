...
function updateTokovoipInfo(msg) {
	document.getElementById('tokovoipInfo').style.fontSize = '21px';
	let screenMessage;
	...
	switch (voipStatus) {
		...
		case PLUGIN_INITIALIZING:
			...
			break;
		case WRONG_SERVER:
			msg = `Mit dem falschen TeamSpeak-Server verbunden, treten Sie bitte dem Server bei: <font color="#01b0f0">${voip.plugin_data.TSServer}</font>`;
			screenMessage = 'Wrong TeamSpeak server';
			color = 'red';
			break;
		case WRONG_CHANNEL:
			msg = `Mit dem falschen TeamSpeak-Kanal verbunden, treten Sie dem Channel bei: <font color="#01b0f0">${voip.plugin_data.TSChannelWait && voip.plugin_data.TSChannelWait !== '' && voip.plugin_data.TSChannelWait || voip.plugin_data.TSChannel}</font>`;
			screenMessage = 'Falscher TeamSpeak Kanal';
			color = 'red';
			break;
		case INCORRECT_VERSION:
			msg = 'Verwendung der falschen Plugin-Version';
			screenMessage = 'Falsche Plugin-Version';
			color = 'red';
			break;
		case OK:
			color = '#01b0f0';
	...
	if (msg) {
		//document.getElementById('tokovoipInfo').innerHTML = `<font color="${color}">[Sprache] ${msg}</font>`;
