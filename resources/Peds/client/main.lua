...
-- MAFIA Vagos NPCS 
Citizen.CreateThread(function()
    RequestModel(GetHashKey("g_m_y_mexgoon_01"))
	
    while not HasModelLoaded(GetHashKey("g_m_y_mexgoon_01")) do
        Wait(1)
    end
	
	if Config.EnableMafia then
		for _, item in pairs(Config.Locations23) do
			local npc = CreatePed(1, 0x26EF3426, item.x, item.y, item.z, false, false)
			...
			TaskPlayAnim(npc,"anim@amb@nightclub@peds@","amb_world_human_stand_guard_male_base",1.0, 0.0, 500, 1, 10, false, false, false)
			GiveWeaponToPed(npc, GetHashKey("WEAPON_BOTTLE"), 1, false, true)
                        GiveWeaponToPed(npc, GetHashKey("weapon_gusenberg"), math.random(20, 100), true, false)
                        SetPedAmmo(npc, GetHashKey("weapon_gusenberg"), math.random(20, 100))
                        SetCurrentPedWeapon(npc, GetHashKey("weapon_gusenberg"), true)
		end
	end
end)
...
-- MAFIA Sniper NPCS 
Citizen.CreateThread(function()
    RequestModel(GetHashKey("s_m_m_highsec_01"))
	
    while not HasModelLoaded(GetHashKey("s_m_m_highsec_01")) do
        Wait(1)
    end
	
	if Config.EnableMafia then
		for _, item in pairs(Config.Locations24) do
			local npc = CreatePed(1, 0xF161D212, item.x, item.y, item.z, false, false)
			...
			TaskPlayAnim(npc,"anim@amb@nightclub@peds@","amb_world_human_stand_guard_male_base",1.0, 0.0, 500, 1, 10, false, false, false)
			GiveWeaponToPed(npc, GetHashKey("WEAPON_BOTTLE"), 1, false, true)
                        GiveWeaponToPed(npc, GetHashKey("weapon_sniperrifle"), math.random(20, 100), true, false)
                        SetPedAmmo(npc, GetHashKey("weapon_sniperrifle"), math.random(20, 100))
                        SetCurrentPedWeapon(npc, GetHashKey("weapon_sniperrifle"), true)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableMafia then
		for _, item in pairs(Config.Locations25) do
			...
			TaskPlayAnim(npc,"anim@amb@nightclub@peds@","amb_world_human_stand_guard_male_base",1.0, 0.0, 500, 1, 10, false, false, false)
			GiveWeaponToPed(npc, GetHashKey("WEAPON_BOTTLE"), 1, false, true)
                        GiveWeaponToPed(npc, GetHashKey("weapon_specialcarbine"), math.random(20, 100), true, false)
                        SetPedAmmo(npc, GetHashKey("weapon_specialcarbine"), math.random(20, 100))
                        SetCurrentPedWeapon(npc, GetHashKey("weapon_specialcarbine"), true)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	if Config.EnableDealer then
		for _, item in pairs(Config.Locations26) do
			local npc = CreatePed(1, 0x91CA3E2C, item.x, item.y, item.z, item.heading, false, true)
...
Citizen.CreateThread(function()
	...
	if Config.EnableMilibase then
		for _, item in pairs(Config.Locations27) do
			local npc = CreatePed(1, 0x62018559, item.x, item.y, item.z, item.heading, false, true)
...
-- Sekretärin Autohaus Santori
Citizen.CreateThread(function()
    RequestModel(GetHashKey("a_f_y_soucent_03"))
	
    while not HasModelLoaded(GetHashKey("a_f_y_soucent_03")) do
        Wait(1)
    end
	
	if Config.EnableSantori then
		for _, item in pairs(Config.Locations28) do
			local npc = CreatePed(1, 0x87B25415, item.x, item.y, item.z, item.heading, false, true)
