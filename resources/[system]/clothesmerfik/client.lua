...
function OpenActionMenuInteraction(target)
	...
	table.insert(elements, {label = ('Kleidung Anziehen'), value = 'ubie'})
	table.insert(elements, {label = ('Shirt Ausziehen'), value = 'tul'})
	table.insert(elements, {label = ('Hose Ausziehen'), value = 'spo'})
	table.insert(elements, {label = ('Schuhe Ausziehen'), value = 'but'})
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(0)
    if IsControlJustReleased(0, 11) and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'action_menu') then
