...
Fwv = function (entity)
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
      ...
      if IsControlJustReleased(0, 344) and GetLastInputMethod(0) then
        beltOn = not beltOn 
        if beltOn then 
		  TriggerEvent("pNotify:SendNotification", {text = "Anschnallen", type = "success", timeout = 1400, layout = "centerLeft"})
		  ...
		  TriggerEvent("pNotify:SendNotification", {text = "Abschnallen", type = "error", timeout = 1400, layout = "centerLeft"})
