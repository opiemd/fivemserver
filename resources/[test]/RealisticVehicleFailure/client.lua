...
AddEventHandler('iens:repair', function()
	if isPedDrivingAVehicle() then
		...
		if IsNearMechanic() then
			...
			notification("~g~Du hast Dein Auto Repariert!")
			return
		end
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				notification("~g~" .. repairCfg.fixMessages[fixMessagePos] .. ", Jetzt fahr zu einem Mechaniker!")
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
			else
				notification("~r~Ihr Fahrzeug wurde zu stark beschädigt. Reparatur nicht möglich!")
			end
		else
		...
		notification("~y~Sie müssen sich in einem Fahrzeug befinden, um es reparieren zu können")
	end
end)

RegisterNetEvent('iens:notAllowed')
AddEventHandler('iens:notAllowed', function()
	notification("~r~Sie haben keine Berechtigung zur Reparatur von Fahrzeugen")
end)
