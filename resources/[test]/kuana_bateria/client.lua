...
local recarregar = {
	{x = -42.74, y = -1754.99, z = 28.25},
	{x = 25.58, y = -1342.23, z = 28.35}, -- recharge place
}
...
Citizen.CreateThread(function ()
	while true do
		...
		for _,v in pairs(recarregar) do
			...
			if distance <= 5 then
				Drawing.draw3DText(v.x, v.y, v.z, "[~g~Ladezone~w~] ~n~Handy kann hier  ~b~Aufgeladen~w~ werden. ~n~Stell Dich hier ~y~hin !~w~", 4, 0.15, 0.05, 255, 255, 255, 255)
				DrawMarker(27, v.x, v.y, v.z + 0.2, 0, 0, 0, 0, 0, 0, 0.5,0.5,0.5, 0, 232, 255, 155, 0, 0, 0, 0, 0, 0, 0)
			end
			if bateria < 99 and distance <= 1.3 then
				ESX.ShowNotification("[~g~Handy Akku~w~]~n~Telemovel a carregar. Bateria: ~g~"..bateria.."%~w~")
				TriggerServerEvent("kuana:updatebateriat")
				Citizen.Wait(2000)
		...
		if (IsControlJustReleased(1, teclatele)) then
			...
			ESX.ShowNotification("[~g~Handy Akku~w~]~n~Dein Telefon hat "..colorbat..""..bateria.."%~w~~n~ Akkuleistung")
		end
	end
end)
