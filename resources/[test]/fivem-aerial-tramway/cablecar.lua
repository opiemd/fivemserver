...
function GivePlayerOptionToJoinMyCablecar(cablecar, moving)
    ...
    if not cablecar.is_player_seated then
        ...
        local dist = #(pos - plypos)
        if dist < 3.0 then
            DrawCablecarText3D("Drücke ~g~E ~w~um in die Seilbahn einzusteigen", pos.x, pos.y, pos.z + 1.0)
        ...
        if not moving then
            DrawCablecarText3D("Drücke ~g~E ~w~um aus der Seilbahn auszusteigen", pos.x, pos.y, pos.z + 1.0)
            if IsControlJustPressed(0, 38) then
                cablecar.is_player_seated = false
                DetachEntity(ply, 0, 0)
            end
