		...
		while true do
			...
			if dstCheck <= 10.0 then
				...
				if dstCheck <= 4.2 then
					ESX.Game.Utils.DrawText3D(SellPos, "[E] Menü Öffnen", 0.4)
					if IsControlJustPressed(0, 38) then
						if IsPedInAnyVehicle(ped, false) then
							OpenSellMenu(GetVehiclePedIsUsing(ped))
						else
							ESX.ShowNotification("Du musst in einem ~g~Auto~s~ sitzen")
						end
					end
				end
			end
					...
					if dstCheck <= 2.0 then
						...
						if IsControlJustPressed(0, 38) then
							if IsPedInVehicle(ped, Config.VehiclePositions[i]["entityId"], false) then
								...
								ESX.ShowNotification("Du musst im ~g~Auto~s~ sitzen!")
							end
						end
					end
				end
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	if not buyVehicle then
		if price ~= nil then
			table.insert(elements, { ["label"] = "Preis ändern - " .. price .. " $", ["value"] = "price" })
			table.insert(elements, { ["label"] = "Zum Verkauf anbieten", ["value"] = "sell" })
		else
			table.insert(elements, { ["label"] = "Preis festlegen - $", ["value"] = "price" })
		end
	else
		table.insert(elements, { ["label"] = "Kaufen " .. price .. " - $", ["value"] = "buy" })

		if owner then
			table.insert(elements, { ["label"] = "Auto Entfernen", ["value"] = "remove" })
		end
	end
	...
	function(data, menu)
		...
		elseif action == "sell" then
			...
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:isVehicleValid", function(valid)
				...
				if valid then
					DeleteVehicle(veh)
					ESX.ShowNotification("Du setzt das ~g~Auto~s~ zum Verkauf für - " .. price .. " $")
					menu.close()
				else
					ESX.ShowNotification("Du musst der ~r~Besitzer~s~ des ~g~Auto~s~ sein! / Es sind  ~r~bereist~s~ " .. #Config.VehiclePositions .. " Autos im Verkauf!")
				end
		...
		elseif action == "buy" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					ESX.ShowNotification("Du ~g~kaufst~s~ das Auto für " .. price .. " $")
					menu.close()
				else
					ESX.ShowNotification("Du hast ~r~nicht~s~ hgenügend bargeld, es fehlen dir " .. price - totalMoney .. " $")
				end
		...
		elseif action == "remove" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					ESX.ShowNotification("Du hast das Auto ~g~entfernt~s~")
					menu.close()
				end
