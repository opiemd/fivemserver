...
AddEventHandler('esx_bikeshop:setVehicleOwned', function(vehicleProps)
	...
	MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, Type) VALUES (@owner, @plate, @vehicle, @Type)',
	{
		...
		TriggerClientEvent('esx:showNotification', _source, _U('bike_belongs', vehicleProps.plate))
	end)
end)
...
ESX.RegisterServerCallback('esx_bikeshop:resellVehicle', function(source, cb, plate, model)
	...
	if resellPrice == 0 then
		print(('esx_bikeshop: %s attempted to sell an unknown vehicle!'):format(GetPlayerIdentifiers(source)[1]))
		cb(false)
	end
		...
		if result[1] then -- does the owner match?
			...
			if vehicle.model == model then
				if vehicle.plate == plate then
					...
					print(('esx_bikeshop: %s attempted to sell an vehicle with plate mismatch!'):format(xPlayer.identifier))
					cb(false)
				end
			else
				print(('esx_bikeshop: %s attempted to sell an vehicle with model mismatch!'):format(xPlayer.identifier))
				cb(false)
			end
		else
			cb(false)
