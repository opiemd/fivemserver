...
AddEventHandler('esx_opium:hasExitedMarker', function(zone)
        ...
        TriggerServerEvent('esx_opium:stopHarvestTurtle')
        TriggerServerEvent('esx_opium:stopTransformTurtle')
        TriggerServerEvent('esx_opium:stopSellTurtle')
end)
...
AddEventHandler('esx_opium:ReturnInventory', function(turtleNbr, turtlepNbr, jobName, currentZone)
	turtleQTE       = turtleNbr
...
Citizen.CreateThread(function()
    while true do
        ...
        if isInMarker and isInZone then
            TriggerEvent('esx_opium:hasEnteredMarker', 'exitMarker')
        end
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, Keys['E']) then
                ...
                if CurrentAction == 'exitMarker' then
                    ...
                    TriggerEvent('esx_opium:freezePlayer', false)
                    TriggerEvent('esx_opium:hasExitedMarker', lastZone)
                    Citizen.Wait(15000)
                    elseif CurrentAction == 'turtle_harvest' then
                        TriggerServerEvent('esx_opium:startHarvestTurtle')
                    elseif CurrentAction == 'turtle_treatment' then
                        TriggerServerEvent('esx_opium:startTransformTurtle')
                    elseif CurrentAction == 'turtle_resell' then
                        TriggerServerEvent('esx_opium:startSellTurtle')
                else
                    isInZone = false
                end

                if isInZone then
                    TriggerEvent('esx_opium:freezePlayer', true)
                end
...
RegisterNetEvent('esx_opium:freezePlayer')
AddEventHandler('esx_opium:freezePlayer', function(freeze)
