...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if PlayerData.job ~= nil and (PlayerData.job.name == 'unemployed' or PlayerData.job.name == 'offpolice' or PlayerData.job.name == 'offmechanic' or PlayerData.job.name == 'offambulance' or PlayerData.job.name == 'offbennys' or PlayerData.job.name == 'mafia') then
...
function removePlayerTattos()
	ClearPedDecorations(PlayerPedId())
	
	count = #currentTattoos
	for i=0, count do currentTattoos[i]=nil end
	ESX.TriggerServerCallback('esx_tattooshop:removePlayerTattoos', function(success)
