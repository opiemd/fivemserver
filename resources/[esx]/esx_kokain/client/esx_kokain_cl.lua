...
AddEventHandler('esx_kokain:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_kokain:hasExitedMarker', function(zone)

        CurrentAction = nil
        ESX.UI.Menu.CloseAll()

        TriggerServerEvent('esx_kokain:stopHarvestTurtle')
        TriggerServerEvent('esx_kokain:stopTransformTurtle')
        TriggerServerEvent('esx_kokain:stopSellTurtle')
end)
...
RegisterNetEvent('esx_kokain:ReturnInventory')
AddEventHandler('esx_kokain:ReturnInventory', function(turtleNbr, turtlepNbr, jobName, currentZone)
	turtleQTE       = turtleNbr
	turtle_meatQTE = turtlepNbr
	myJob         = jobName
	TriggerEvent('esx_kokain:hasEnteredMarker', currentZone)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isInMarker and not HasAlreadyEnteredMarker then
            ...
            TriggerServerEvent('esx_kokain:GetUserInventory', currentZone)
        end

        if not isInMarker and HasAlreadyEnteredMarker then
            HasAlreadyEnteredMarker = false
            TriggerEvent('esx_kokain:hasExitedMarker', LastZone)
        end

        if isInMarker and isInZone then
            TriggerEvent('esx_kokain:hasEnteredMarker', 'exitMarker')
        end
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, Keys['E']) then
                ...
                if CurrentAction == 'exitMarker' then
                    ...
                    TriggerEvent('esx_kokain:freezePlayer', false)
                    TriggerEvent('esx_kokain:hasExitedMarker', lastZone)
                    Citizen.Wait(15000)
                    elseif CurrentAction == 'turtle_harvest' then
                        TriggerServerEvent('esx_kokain:startHarvestTurtle')
                    elseif CurrentAction == 'turtle_treatment' then
                        TriggerServerEvent('esx_kokain:startTransformTurtle')
                    elseif CurrentAction == 'turtle_resell' then
                        TriggerServerEvent('esx_kokain:startSellTurtle')
                else
                    isInZone = false
                end

                if isInZone then
                    TriggerEvent('esx_kokain:freezePlayer', true)
                end
...
RegisterNetEvent('esx_kokain:freezePlayer')
AddEventHandler('esx_kokain:freezePlayer', function(freeze)
