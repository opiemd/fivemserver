...
ESX.RegisterServerCallback('esx_tattooshop:removePlayerTattoos', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer then
		MySQL.Async.fetchAll('UPDATE users SET tattoos = NULL WHERE identifier = @identifier', {
			['@identifier'] = xPlayer.identifier
		})
