...
AddEventHandler('esx_SodaMachine:Random', function(prop_name)
	...
	if lottery <= 6 then
		if not IsAnimated then
			...
			Citizen.CreateThread(function()
				...
				ESX.ShowNotification('Automat Arbeiten ... Bitte warten.')
				...
				Citizen.Wait(700)
				ESX.ShowNotification('Du hast gerade ~y~1x~s~ ~b~Getraenke Dose~s~ gekauft fuer ~r~$1~s~')
	...
	if lottery == 7 then
			Citizen.CreateThread(function()
			ESX.ShowNotification('Dieser Automat ist außer Betrieb')
			end)
		end
	if lottery == 8 then
			Citizen.CreateThread(function()
			ESX.ShowNotification('Das Produckt ist aus diesem Automaten')
			end)
		end
	if lottery == 9 then
			Citizen.CreateThread(function()
			...
			TriggerEvent('InteractSound_CL:PlayOnOne', 'SodaMachine', 1.0)
			Citizen.Wait(5000)
			ESX.ShowNotification('Es gab einen fehler und der Automat hat dein geld geschluckt.')
			FreezeEntityPosition(ped, false)
			end)
		end
end)