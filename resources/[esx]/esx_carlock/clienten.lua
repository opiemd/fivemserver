...
Citizen.CreateThread(function()
  ...
  while true do
	...
	if (IsControlJustPressed(1, 303)) then
		...
		notowned = 0
		if #cars == 0 then
			ESX.ShowNotification("Keine Fahrzeuge in der Nähe.")
		else
			for j=1, #cars, 1 do
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							ESX.ShowNotification('Du hast dein ~y~'..vehicleLabel..' ~r~Abgeschlossen~s~')
						...
						elseif lock == 2 then
							...
							ESX.ShowNotification('Du hast dein ~y~'..vehicleLabel..' ~g~Aufgeschlossen~s~')
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							hasAlreadyLocked = true
