		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Verlasse dein Fahrzeug zum Tanken")
				elseif IsFueling then
					local position = GetEntityCoords(vehicle)

					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Drücke ~g~E ~w~um das Auftanken abzubrechen. $~r~" .. price .. " ~w~ zu bezahlen")
					...
					DisableControlAction(0, 44, false) -- Taking Cover (Q)
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Der Tank ist bereits voll.")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Du hast nicht genug Geld bei dir.")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Drücke ~g~E ~w~um dein Fahrzeug zu tanken. $~r~0.5/~w~Dollar")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Drücke ~g~E ~w~um das Tanken abzubrechen. Derzeit bei: " .. fuel .. "% - Jerry Can: " .. jerrycan)
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Drücke ~g~E ~w~um dein Fahrzeug mit dem Benzinkanister zu befüllen.")
...
function DisplayHud()
	...
	if Config.ShouldDisplayHud and IsPedInAnyVehicle(playerPed, false) and GetSeatPedIsIn(playerPed) == -1 then
		...
		DrawAdvancedText(0.2195 - x, 0.7720 - y, -0.055, 0.0028, 0.4, fuel, 255, 255, 255, 255, 6, 1)



		DrawAdvancedText(0.148 - x, 0.7720 - y, -0.090, 0.0028, 0.4, "                                    Benzin", 255, 255, 255, 255, 6, 1)
	end
end
