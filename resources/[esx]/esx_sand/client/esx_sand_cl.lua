...
AddEventHandler('esx_sand:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_sand:hasExitedMarker', function(zone)

        CurrentAction = nil
        ESX.UI.Menu.CloseAll()

        TriggerServerEvent('esx_sand:stopHarvestTurtle')
        TriggerServerEvent('esx_sand:stopTransformTurtle')
        TriggerServerEvent('esx_sand:stopSellTurtle')
end)
...
RegisterNetEvent('esx_sand:ReturnInventory')
AddEventHandler('esx_sand:ReturnInventory', function(turtleNbr, turtlepNbr, jobName, currentZone)
	turtleQTE       = turtleNbr
	turtle_meatQTE = turtlepNbr
	myJob         = jobName
	TriggerEvent('esx_sand:hasEnteredMarker', currentZone)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isInMarker and not HasAlreadyEnteredMarker then
            ...
            TriggerServerEvent('esx_sand:GetUserInventory', currentZone)
        end

        if not isInMarker and HasAlreadyEnteredMarker then
            HasAlreadyEnteredMarker = false
            TriggerEvent('esx_sand:hasExitedMarker', LastZone)
        end

        if isInMarker and isInZone then
            TriggerEvent('esx_sand:hasEnteredMarker', 'exitMarker')
        end
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, Keys['E']) then
                ...
                if CurrentAction == 'exitMarker' then
                    ...
                    TriggerEvent('esx_sand:freezePlayer', false)
                    TriggerEvent('esx_sand:hasExitedMarker', lastZone)
                    Citizen.Wait(15000)
                    elseif CurrentAction == 'turtle_harvest' then
                        TriggerServerEvent('esx_sand:startHarvestTurtle')
                    elseif CurrentAction == 'turtle_treatment' then
                        TriggerServerEvent('esx_sand:startTransformTurtle')
                    elseif CurrentAction == 'turtle_resell' then
                        TriggerServerEvent('esx_sand:startSellTurtle')
                else
                    isInZone = false
                end

                if isInZone then
                    TriggerEvent('esx_sand:freezePlayer', true)
                end
...
RegisterNetEvent('esx_sand:freezePlayer')
AddEventHandler('esx_sand:freezePlayer', function(freeze)
