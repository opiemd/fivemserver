...
Citizen.CreateThread(function()
	while true do
		...
		if inVehicle == false then
			...
			if IsPedInAnyVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then
				local essence = GetVehicleFuelLevel(GetVehiclePedIsIn(GetPlayerPed(-1)))
				local percent = essence
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if IsControlJustPressed(1, 74) and IsControlPressed(1, 21) and allowProximityChange then
			local vprox
			if prox <= 2.0 then
				prox = 10.0
				vprox = "normal"
		...
		if IsControlPressed(1, 74) and IsControlPressed(1, 21) then
...
RegisterNetEvent('esx_raidrpui:updateStatus')
AddEventHandler('esx_raidrpui:updateStatus', function(status)
	SendNUIMessage({action = "updateStatus", status = status})
end)

RegisterNetEvent('esx_raidrpui:updateWeight')
AddEventHandler('esx_raidrpui:updateWeight', function(weight)
