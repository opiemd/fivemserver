...
AddEventHandler('esx_heroin:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_heroin:hasExitedMarker', function(zone)

        CurrentAction = nil
        ESX.UI.Menu.CloseAll()

        TriggerServerEvent('esx_heroin:stopHarvestTurtle')
        TriggerServerEvent('esx_heroin:stopTransformTurtle')
        TriggerServerEvent('esx_heroin:stopSellTurtle')
end)
...
RegisterNetEvent('esx_heroin:ReturnInventory')
AddEventHandler('esx_heroin:ReturnInventory', function(turtleNbr, turtlepNbr, jobName, currentZone)
	turtleQTE       = turtleNbr
	turtle_meatQTE = turtlepNbr
	myJob         = jobName
	TriggerEvent('esx_heroin:hasEnteredMarker', currentZone)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isInMarker and not HasAlreadyEnteredMarker then
            ...
            TriggerServerEvent('esx_heroin:GetUserInventory', currentZone)
        end

        if not isInMarker and HasAlreadyEnteredMarker then
            HasAlreadyEnteredMarker = false
            TriggerEvent('esx_heroin:hasExitedMarker', LastZone)
        end

        if isInMarker and isInZone then
            TriggerEvent('esx_heroin:hasEnteredMarker', 'exitMarker')
        end
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, Keys['E']) then
                ...
                if CurrentAction == 'exitMarker' then
                    ...
                    TriggerEvent('esx_heroin:freezePlayer', false)
                    TriggerEvent('esx_heroin:hasExitedMarker', lastZone)
                    Citizen.Wait(15000)
                    elseif CurrentAction == 'turtle_harvest' then
                        TriggerServerEvent('esx_heroin:startHarvestTurtle')
                    elseif CurrentAction == 'turtle_treatment' then
                        TriggerServerEvent('esx_heroin:startTransformTurtle')
                    elseif CurrentAction == 'turtle_resell' then
                        TriggerServerEvent('esx_heroin:startSellTurtle')
                else
                    isInZone = false
                end

                if isInZone then
                    TriggerEvent('esx_heroin:freezePlayer', true)
                end
...
RegisterNetEvent('esx_heroin:freezePlayer')
AddEventHandler('esx_heroin:freezePlayer', function(freeze)
